...
class ListFechamentosService {
  async execute({
    ...
    const query = repo.createQueryBuilder('fechamento').leftJoinAndSelect('fechamento.empresa', 'empresa');

    // 🔎 filtro por empresa_id (select)
    if (empresa_id) {
      query.andWhere('empresa.id = :empresa_id', {
        empresa_id
      });
    }
    ...
    // 🔎 filtro por status
    if (status) {
      query.andWhere('fechamento.status = :status', {
        status
      });
    }
    ...
    if (data_inicial && data_final) {
      ...
      query.andWhere('fechamento.data_fechamento >= :ini', {
        ini: data_inicial
      });
    } else if (data_final) {
      query.andWhere('fechamento.data_fechamento <= :fim', {
        fim: data_final
      });
    }
    ...
    // 🔹 aplica paginação
    const fechamentos = await query.skip((page - 1) * per_page).take(per_page).getMany();
...
var _default = exports.default = ListFechamentosService;