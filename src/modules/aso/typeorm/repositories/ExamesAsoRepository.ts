...
export class ExamesAsoRepository extends Repository<ExameAso>{
  ...
  public async findExamesRealizadosPeriodoEmpresaForaNao(datainicio: string,datafim: string, empresa: string ,tipopagamento: string ): Promise<ExameAso[] | undefined> {
  ...
  const exameAso = await this.find({
    ...
    public async findExamesRealizadosPeriodoConsolidado(datainicio: string,datafim: string, idexame: string ): Promise<ExameAso[] | undefined> {
